        ...
        match self {
            ...
            ScalarValue::Map(v, s) => {
                if let ScalarValue::Struct(_, fields) = v.as_ref() {
                    DataType::Map(
                        Arc::new(Field::new("entries", DataType::Struct(fields.clone()), false)),
                        *s,
                    )
                } else {
    ...
    fn iter_to_array_list(
        ...
        let flat_array =
            if element_arrays.is_empty() {
                return Ok(as_list_array(&make_array(ArrayData::new_empty(data_type)))?.clone());
            } else {
                match arrow::compute::concat(&element_arrays) {
